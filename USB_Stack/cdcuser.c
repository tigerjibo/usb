...
//  numBytesRead = ser_Read ((char *)&BulkBufIn[0], &numBytesAvail);
  ...
  numBytesRead = UARTReceive((LPC_UART_TypeDef *)LPC_UART0, &BulkBufIn[0], sizeof(BulkBufIn));
  // send over USB
 // numBytesRead  = USB_CDC_BUFSIZE;
  if (numBytesRead > 0) {
...
void CDC_BulkOut(void) {
 ...
 // LED_Count();
  ...
  UARTSend((LPC_UART0_TypeDef*)LPC_UART0, &BulkBufOut[0], &numBytesRead);
